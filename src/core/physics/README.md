...
│   └── Body            — Single class tagged by shape (pm2d/rigid2d) and motion (static/kinematic/dynamic)
...
- **`shape`** — `"pm2d"` (point mass: position only, never rotates) or `"rigid2d"` (rigid body: position + angle). Controls the DOF set.
...
Immovable geometry (walls, floors).
...
`createPointMass2D` creates a particle-like body: it has position, velocity and mass but no rotation (`angle` stays 0 and torques are ignored). It takes the same options as `createRigid2D` minus the angular ones. Constraints between point masses use cheaper solver paths, so prefer them for things like chain links or debris that never needs to spin.

```typescript
const spark = createPointMass2D({
  motion: "dynamic",
  mass: 0.1,
  position: [0, 0],
});
spark.addShape(new Particle());
world.bodies.add(spark);
```
...
const world = new World({
  solverConfig: {
    ...
    frictionIterations: 3, // See below
  },
  substeps: 2, // Solve + integrate this many times per step()
});
```

**Friction:** with `frictionIterations: 0` (the default) every friction equation is limited by a fixed slip force, regardless of the `ContactMaterial`'s `friction`. With `frictionIterations > 0` the solver first runs that many iterations to estimate contact forces, then limits each friction equation to `friction × (average normal force of its contacts)` — i.e. proper Coulomb-style friction that scales with how hard the bodies are pressed together.

**Substepping:** broadphase, narrowphase and contact generation run once per `step()`; constraint solving and position integration run `substeps` times at `dt / substeps`. This stiffens long constraint chains without multiplying collision detection cost.

### Solver internals

Every constraint row is an `Equation` with a 6-component Jacobian `G = [vxA, vyA, wA, vxB, vyB, wB]`. The solver partitions equations by which components are structurally non-zero (planar, angular, point-to-point, point-to-rigid, general) and runs a dedicated inner loop for each. See `CLAUDE.md` in this folder for the taxonomy.
...
│   ├── bodyFactories.ts     — Typed factory functions (createRigid2D, createPointMass2D)
│   ├── bodyInterfaces.ts    — Narrowed structural views per shape/motion
│   ├── body-helpers.ts      — Small predicates (isAwake, hasOnlyParticleShapes)
│   ├── ccdUtils.ts          — Continuous collision detection
...
│   ├── Circle.ts, Box.ts, Convex.ts, Capsule.ts, Line.ts, Plane.ts, Particle.ts
├── constraints/
...
├── equations/
│   ├── Equation.ts          — Base constraint equation (general 6-component Jacobian)
│   ├── PlanarEquation2D.ts, AngularEquation2D.ts,
│   │   PointToPointEquation2D.ts, PointToRigidEquation2D.ts — Specialized Jacobian shapes
│   ├── ContactEquation.ts   — Penetration prevention
│   ├── FrictionEquation.ts  — Surface friction
│   └── (rotational lock / motor equations)
├── material/
│   ├── Material.ts          — Material identifier
│   └── ContactMaterial.ts   — Material pair properties
├── solver/
│   ├── GSSolver.ts          — Gauss-Seidel constraint solver
│   └── SolverWorkspace.ts   — Flat per-body / per-equation solver scratch arrays
├── events/
