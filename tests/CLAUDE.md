...
- `window.DEBUG.game` exposes the `Game`. Tests find things with `entities.getById(...)` (`lobby`), `entities.getTagged(...)` (`human`, `zombie`, `lobby_character`, `directory_board`), and by class name (see below).
