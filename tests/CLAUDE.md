...
- The upgrades offered between floors are drawn right after the next level is generated, so they are seeded too. `UPGRADE_OFFER` pins what is offered after level 1; it changes when the pool or rarity weights change (paste the new value, and pick a different upgrade in the test if Steady Aim is no longer offered).
