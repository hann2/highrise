...
- `src/highrise/` — the actual game
  ...
  - `persistence/SaveData.ts` — the one versioned localStorage entry for everything kept between runs (run history, best floor, unlocked characters, which always include `DEFAULT_UNLOCKED_CHARACTERS`, and the encyclopedia's seen flags); tolerant of missing/corrupt data. Put new persistent state here rather than in new keys. `run/RunStats.ts` keeps score during a run and saves the `RunSummary` at game over
  - `encyclopedia/` — what the encyclopedia screen (`menu/Encyclopedia.tsx`, opened from the main menu, pause menu and run summary) lists: `encyclopediaSections.ts` builds each section's entries from the data indexes, and `EncyclopediaTracker` (one per run) marks guns, melee weapons and enemies as seen in `SaveData.seen`; upgrades are marked when offered or taken
