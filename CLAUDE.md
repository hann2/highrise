...
- Stats-as-data: guns, melee weapons, consumables, characters, decorations, zombie variants, and upgrades are plain objects in their own files, collected in an index (`gunStats.ts`, `weapons.ts`, `consumableStats.ts`, `decorations.ts`, `Character.ts`, `upgrades.ts`). macOS file names are case-insensitive, so an index can't sit next to an interface of the same name (`GunStats.ts` vs `gun-stats/gunStats.ts`).
