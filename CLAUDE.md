...
- `src/highrise/` — the actual game
  ...
  - `upgrades/` — the pick-one-of-three pool. Each `Upgrade` changes a human's `PlayerStats` (`human/PlayerStats.ts`: multipliers and rule flags, neutral by default), which `Human`, `Gun`, `Bullet`, enemies, `Flashlight` and `VisionController` read at use time; `GunStats` stay shared and read-only. `LevelController` shows `menu/UpgradeSelect` (with the game paused) between floors, after generating the next level so the draw doesn't affect it
