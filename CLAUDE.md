...
- **Gotcha:** `EntityList` registers _every_ method starting with `on` as an event handler. Don't name an ordinary method `on<SomeEventName>` (e.g. `onGiveWeapon`) or it will be called on dispatch with the event payload. The type checker catches this only when the signatures differ.
