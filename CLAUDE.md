...
- `src/highrise/` — the actual game
  ...
  - `persistence/SaveData.ts` — the one versioned localStorage entry for everything kept between runs (run history, best floor, unlocked characters, which always include `DEFAULT_UNLOCKED_CHARACTERS`); tolerant of missing/corrupt data. Put new persistent state here rather than in new keys. `run/RunStats.ts` keeps score during a run and saves the `RunSummary` at game over
