...
- `src/highrise/` — the actual game
  ...
  - `lobby/` — the hub between runs and the diegetic main menu. `Lobby` builds the hand-made lobby (`levels/level-generation/lobbyGeneration.ts` + `levels/rooms/LobbyRoomTemplate.ts`, which holds the layout), puts the player in the arrival elevator, and stands every other character around (`LobbyCharacterController`: E swaps into an unlocked one; locked ones are silhouettes). Also the `DirectoryBoard` (shows the `RunPlan`), `ReceptionistBob` (a harmless heavy), and the encyclopedia bookcases. No enemies, no fog of war, no run HUD (only the big version of `hud/InteractPrompt`)
